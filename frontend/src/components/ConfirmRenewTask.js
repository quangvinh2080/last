...
const ConfirmRenewTask = ({ task, open, onClose }) => {
  ...
  return (
    <Modal className="text-black space-y-5" open={open}>
      <Modal.Header className="font-bold mb-4">
        <h3 className="font-semibold text-2xl text-gray-800">Renew {task.name}</h3>
        <p className="text-gray-500">Are you sure to renew latest date of the task {task.name} ?</p>
      ...
      <Modal.Body>
        <div className="space-y-5">
          ...
          <div className="space-y-2 text-right">
            <ButtonGroup>
              <Button className="w-20" type="submit" active onClick={() => handleRenew()} loading={isLoading} disabled={isLoading}>
                Yes
              </Button>
...
export default ConfirmRenewTask