...
const UpdateTask = ({ task, open, onClose }) => {
  ...
  const [expectDays, setExpectDays] = useState(task.expected_days);
  const [latestDate, setLatestDate] = useState({
    startDate: task.latest_date,
    endDate: task.latest_date,
  });
  ...
  const handleUpdateTask = async () => {
    try {
      setIsLoading(true);
      const { data: newTask, status } = await updateTask(task._id, { name, description, expected_days: expectDays, latest_date: latestDate.startDate });
      if (status === 200) {
        taskDispatch({ 
          type: 'UPDATE_TASK', 
          value: newTask
